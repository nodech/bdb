...
class DB {
  ...
  batch() {
    ...
    return new Batch(this.binding);
  }

  /**
   * Create a chained batch.
   * @returns {ChainedBatch}
   */
  ...
  chainedBatch() {
    ...
    return new ChainedBatch(this.binding.batch());
  }
...
class ChainedBatch {
  ...
  bucket(prefix) {
    return new ChainedBatch(this.binding, prefix);
  }
...
class Batch {
  ...
  put(key, value) {
    ...
    this.operations.push(new BatchOp('put', concat(this.prefix, key), value));
  ...
  del(key) {
    assert(!this.written, 'Already written.');
    this.operations.push(new BatchOp('del', concat(this.prefix, key)));
    return this;
  }
  ...
  write() {
    ...
    return new Promise((resolve, reject) => {
      this.binding.batch(this.operations, wrap(resolve, reject));
    });
  }
...
class Bucket {
  ...
  batch() {
    return new Batch(this.db.binding, this.prefix);
  }
  ...
  chainedBatch() {
    return new ChainedBatch(this.db.binding.batch(), this.prefix);
  }
